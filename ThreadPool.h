...
public:
  ...
  //调用线程池
  void Call(int (*pFunc)(void  *), void *pPara = NULL)
  {
    assert(pFunc);
...
protected:
    ...
    for(;;)
    {
      ...
      if(fHasJob && pJob)
      {
		  if(pThread)
		  {
			...
			if(pJob)
				int ret= pJob->_pFunc(pJob->_pPara); //运行用户作业
			delete pJob; 
			pJob=0;
  ...
  //调用用户对象虚函数
  static int CallProc(void *pPara) 
  {
  ...
  struct JobItem 
  {
    int (*_pFunc)(void  *);//函数
    void *_pPara; //参数
    JobItem(int (*pFunc)(void  *) = NULL, void *pPara = NULL) : _pFunc(pFunc), _pPara(pPara) { };
  };
  //线程池中的线程结构
  struct ThreadItem
  {
